...
function out=fractionalMgate(NetProp,V,t,alpha)
...
%     Ngatememory(a,:)=0;
   ...
   % betan=(4.*exp(-(V-v0)./18));
    nf=@(x,V,v0)(((2.5-0.1.*(V-v0))./(exp(2.5-0.1.*(V-v0))-1))*(1-x)-(4.*exp(-(V-v0)./18))*x);
    k1=nf(nV(a),V(a),v0);
...
% mf=@(Mm,V)(((2.5-0.1.*(V-v0))./(exp(2.5-0.1.*(V-v0))-1))*(1-Mm)-(4.*exp(-(V-v0)./18))*Mm);
    ...
    alphan=((2.5-0.1.*(V(a)-v0))./(exp(2.5-0.1.*(V(a)-v0))-1));
    betan=(4.*exp(-(V(a)-v0)./18));
